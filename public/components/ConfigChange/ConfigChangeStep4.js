...
class ConfigChangeStep4 extends React.Component {
  ...
  okConfirm = () => {
    this.setState({ confirmDiagOpen: false });
    this.props.liveRunSyncStart({
      dry_run: false,
      comment: this.state.job_comment,
      ticket_ref: this.state.job_ticket_ref,
      confirm_mode: this.state.confirmMode,
    });
  ...
  componentDidMount() {
    ...
    getData(url, credentials)
      ...
      .catch((error) => {
        console.log(
          "API does not support setteings/server to get default commit confirm mode",
        );
      });
  }
  ...
  render() {
    const progressData = this.props.liveRunProgressData;
    const jobStatus = this.props.liveRunJobStatus;
    const { jobId } = this.props;
    const { dryRunJobStatus } = this.props;
    const { confirmRunProgressData } = this.props;
    const { confirmJobId } = this.props;
    const confirmJobStatus = this.props.confirmRunJobStatus;
    const warnings = [];
    ...
    if (!commitButtonDisabled) {
      ...
      if (
        this.props.dryRunChangeScore &&
        this.props.dryRunChangeScore > warnChangeScore
      ) {
        warnings.push(
          <Popup
            key="popup2"
