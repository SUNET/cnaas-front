...
class SyncStatus extends React.Component {
  ...
  getDeviceList() {
    if (this.props.target.hostname !== undefined) {
      ...
      const url =
        process.env.API_URL +
        "/api/v1.0/devices?filter[hostname]=" +
        this.props.target.hostname +
        "&filter[state]=MANAGED&per_page=1";
      ...
      const url_devices =
        process.env.API_URL +
        "/api/v1.0/devices?filter[synchronized]=false&filter[state]=MANAGED&per_page=1000";
      getData(url_devices, credentials).then((data_devices) => {
        const url_group =
          process.env.API_URL + "/api/v1.0/groups/" + this.props.target.group;
